...
<template>
 ...
 placeholder="Name"
 class="kv-key"
 @input="v.key = ($event.target as HTMLInputElement).value; update('variables', [...(node as TestPlan).variables])"
 />
 <input
 type="text"
 :value="v.value"
 placeholder="Value"
 class="kv-value"
 @input="v.value = ($event.target as HTMLInputElement).value; update('variables', [...(node as TestPlan).variables])"
 />
 <button
 class="kv-remove"
 @click="() => {const vars = [...(node as TestPlan).variables]; vars.splice(i, 1); update('variables', vars)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const vars = [...(node as TestPlan).variables]; vars.push({ key: '', value: '' }); update('variables', vars)}"
 >
 + Add Variable
 </button>
 </div>
 ...
 style="flex: 2"
 @input="l.name = ($event.target as HTMLInputElement).value; update('listeners', [...(node as TestPlan).listeners])"
 />
 <button
 class="kv-remove"
 @click="() => {const arr = [...(node as TestPlan).listeners]; arr.splice(i, 1); update('listeners', arr)}"
 >
 x
 </button>
 </div>
 ...
 :value="a.metric"
 class="kv-key"
 @change="a.metric = ($event.target as HTMLSelectElement).value; update('assertions', [...(node as TestPlan).assertions])"
 >
 ...
 class="kv-key"
 style="width: 60px"
 @change="a.operator = ($event.target as HTMLSelectElement).value; update('assertions', [...(node as TestPlan).assertions])"
 >
 ...
 class="kv-value"
 style="width: 80px"
 @input="a.value = parseFloat(($event.target as HTMLInputElement).value) || 0; update('assertions', [...(node as TestPlan).assertions])"
 />
 <button
 class="kv-remove"
 @click="() => {const arr = [...(node as TestPlan).assertions]; arr.splice(i, 1); update('assertions', arr)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const arr = [...(node as TestPlan).assertions]; arr.push({ metric: 'errorRate', operator: 'lt', value: 5 }); update('assertions', arr)}"
 >
 + Add Assertion
 </button>
 </div>
