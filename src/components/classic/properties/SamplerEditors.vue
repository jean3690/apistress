...
<template>
 ...
 @change="update('auth', {type: ($event.target as HTMLSelectElement).value,username: (node as HttpSampler).auth?.username || '',password: (node as HttpSampler).auth?.password || '',token: (node as HttpSampler).auth?.token || '',})"
 >
 ...
 @input="update('auth', { ...(node as HttpSampler).auth, username: ($event.target as HTMLInputElement).value })"
 />
 </div>
 <div class="prop-row">
 ...
 @input="update('auth', { ...(node as HttpSampler).auth, password: ($event.target as HTMLInputElement).value })"
 />
 </div>
 </template>
 ...
 class="kv-key"
 list="http-headers"
 @input="h.key = ($event.target as HTMLInputElement).value; update('headers', [...(node as HttpSampler).headers])"
 />
 <input
 type="text"
 :value="h.value"
 placeholder="Value"
 class="kv-value"
 @input="h.value = ($event.target as HTMLInputElement).value; update('headers', [...(node as HttpSampler).headers])"
 />
 <button
 class="kv-remove"
 @click="() => {const hdrs = [...(node as HttpSampler).headers]; hdrs.splice(i, 1); update('headers', hdrs)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const hdrs = [...(node as HttpSampler).headers]; hdrs.push({ key: '', value: '' }); update('headers', hdrs)}"
 >
 + Add Header
 </button>
 </div>
 ...
 placeholder="Name"
 class="kv-key"
 @input="p.key = ($event.target as HTMLInputElement).value; update('queryParams', [...(node as HttpSampler).queryParams])"
 />
 <input
 type="text"
 :value="p.value"
 placeholder="Value"
 class="kv-value"
 @input="p.value = ($event.target as HTMLInputElement).value; update('queryParams', [...(node as HttpSampler).queryParams])"
 />
 <button
 class="kv-remove"
 @click="() => {const qp = [...(node as HttpSampler).queryParams]; qp.splice(i, 1); update('queryParams', qp)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const qp = [...(node as HttpSampler).queryParams]; qp.push({ key: '', value: '' }); update('queryParams', qp)}"
 >
 + Add Query Param
 </button>
 </div>
 ...
 class="kv-key"
 list="http-headers"
 @input="h.key = ($event.target as HTMLInputElement).value; update('headers', [...(node as GraphQlSampler).headers])"
 />
 <input
 type="text"
 :value="h.value"
 placeholder="Value"
 class="kv-value"
 @input="h.value = ($event.target as HTMLInputElement).value; update('headers', [...(node as GraphQlSampler).headers])"
 />
 <button
 class="kv-remove"
 @click="() => {const hdrs = [...(node as GraphQlSampler).headers]; hdrs.splice(i, 1); update('headers', hdrs)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const hdrs = [...(node as GraphQlSampler).headers]; hdrs.push({ key: '', value: '' }); update('headers', hdrs)}"
 >
 + Add Header
 </button>
 </div>
 ...
 class="kv-key"
 list="http-headers"
 @input="h.key = ($event.target as HTMLInputElement).value; update('headers', [...(node as SseSampler).headers])"
 />
 <input
 type="text"
 :value="h.value"
 placeholder="Value"
 class="kv-value"
 @input="h.value = ($event.target as HTMLInputElement).value; update('headers', [...(node as SseSampler).headers])"
 />
 <button
 class="kv-remove"
 @click="() => {const hdrs = [...(node as SseSampler).headers]; hdrs.splice(i, 1); update('headers', hdrs)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const hdrs = [...(node as SseSampler).headers]; hdrs.push({ key: '', value: '' }); update('headers', hdrs)}"
 >
 + Add Header
 </button>
 </div>
 ...
 class="kv-key"
 list="http-headers"
 @input="h.key = ($event.target as HTMLInputElement).value; update('headers', [...(node as WebSocketSampler).headers])"
 />
 <input
 type="text"
 :value="h.value"
 placeholder="Value"
 class="kv-value"
 @input="h.value = ($event.target as HTMLInputElement).value; update('headers', [...(node as WebSocketSampler).headers])"
 />
 <button
 class="kv-remove"
 @click="() => {const hdrs = [...(node as WebSocketSampler).headers]; hdrs.splice(i, 1); update('headers', hdrs)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const hdrs = [...(node as WebSocketSampler).headers]; hdrs.push({ key: '', value: '' }); update('headers', hdrs)}"
 >
 + Add Header
 </button>
 </div>
 ...
 class="kv-key"
 list="http-headers"
 @input="h.key = ($event.target as HTMLInputElement).value; update('metadata', [...(node as GrpcSampler).metadata])"
 />
 <input
 type="text"
 :value="h.value"
 placeholder="Value"
 class="kv-value"
 @input="h.value = ($event.target as HTMLInputElement).value; update('metadata', [...(node as GrpcSampler).metadata])"
 />
 <button
 class="kv-remove"
 @click="() => {const hdrs = [...(node as GrpcSampler).metadata]; hdrs.splice(i, 1); update('metadata', hdrs)}"
 >
 x
 </button>
 </div>
 <button
 class="kv-add"
 @click="() => {const hdrs = [...(node as GrpcSampler).metadata]; hdrs.push({ key: '', value: '' }); update('metadata', hdrs)}"
 >
 + Add Metadata
 </button>
 </div>
